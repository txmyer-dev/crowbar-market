...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Running the Upgrade workflow in the SNAPUpgrade skill to check for upgrades"}' \
  > /dev/null 2>&1 &
```

Running the **Upgrade** workflow in the **SNAPUpgrade** skill to check for upgrades...

**Primary workflow for SNAPUpgrade skill.** Generates prioritized upgrade recommendations by running two parallel agent threads: user context analysis and source collection.
...
This workflow executes the core SNAPUpgrade pattern:

1. **Thread 1:** Analyze user context (TELOS, projects, recent work, SNAP state)
...
"Analyze the current state of the user's SNAP system:
...
Return format for EACH technique:
{
  ...
  'current_pai_gap': '[What SNAP currently does/lacks that this addresses]',
  'implementation_file': '[Path to SNAP file this would change]',
...
1. Load channel config:
   bun ~/.claude/skills/SNAP/Tools/LoadSkillConfig.ts ~/.claude/skills/SNAPUpgrade youtube-channels.json
...
3. Compare against state:
   cat ~/.claude/skills/SNAPUpgrade/State/youtube-videos.json

4. For NEW videos, extract transcripts:
   bun ~/.claude/skills/SNAP/Tools/GetTranscript.ts '<video-url>'
...
5. CRITICAL - For each transcript, extract SPECIFIC TECHNIQUES:
   ...
   - Identify what SNAP component this applies to
...
Return format for EACH technique found:
{
  ...
  'pai_relevance': '[Which SNAP component this could improve]'
}
...
1. Look in ~/.claude/skills/SNAP/USER/SKILLCUSTOMIZATIONS/SNAPUpgrade/
...
**Extract techniques. Show the actual content. Explain SNAP relevance.**
...
[Describe the specific benefit to our SNAP system - which component improves, what gap it fills, what becomes possible. Must be exactly 16-32 words.]
...
2. **MAP TO SNAP** - Every technique must name a specific SNAP file or component it improves
3. **TWO MANDATORY DESCRIPTIONS (16-32 words each):**
   ...
   - **How It Helps SNAP:** Describe the specific benefit - which component improves, what gap it fills
...
**How It Helps SNAP (16-32 words):**
Our session documentation workflows had manual session ID extraction hacks. Native substitution means cleaner code and reliable session tracking across all SNAP workflows.
...
**This workflow implements the core SNAPUpgrade value proposition: understanding YOU first, discovering what's new second, then connecting them into actionable, personalized upgrades.**
