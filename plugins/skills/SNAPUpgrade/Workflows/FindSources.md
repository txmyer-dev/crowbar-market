...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Running the FindSources workflow in the SNAPUpgrade skill to discover sources"}' \
  > /dev/null 2>&1 &
```

Running the **FindSources** workflow in the **SNAPUpgrade** skill to discover sources...
...
This workflow helps identify new sources worth monitoring for SNAP-relevant updates:
...
- Content relevance to SNAP infrastructure
...
**URL:** [url]
**Relevance:** [Why this matters for SNAP]
...
Edit `~/.claude/skills/SNAP/USER/SKILLCUSTOMIZATIONS/SNAPUpgrade/youtube-channels.json`

### For Other Sources:
Currently, non-YouTube sources are monitored via the base `sources.json`.
To request additions to base Anthropic monitoring, note them for next SNAP release.
```
...
cat ~/.claude/skills/SNAP/USER/SKILLCUSTOMIZATIONS/SNAPUpgrade/youtube-channels.json
