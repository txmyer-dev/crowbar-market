 ...
 * AI-powered recommendations for improving SNAP infrastructure.
 *
 * Usage:
...
// Config
const HOME = homedir();
const SKILL_DIR = join(HOME, '.claude', 'skills', 'SNAPUpgrade');
...
async function fetchGitHubRepo(source: Source, state: State): Promise<Update[]> {
  ...
  const headers: Record<string, string> = {
    ...
    'User-Agent': 'SNAP-Anthropic-Monitor'
  };
...
// Recommendation engine - SNAP SNAP ecosystem focused
function generateRecommendation(update: Update): string {
  const { source, type, title, category } = update;
  const titleLower = title.toLowerCase();

  // SKILLS - Critical for SNAP's skill system
  if (titleLower.includes('skill') || titleLower.includes('skills')) {
    return `**SNAP Impact:** CRITICAL for skills ecosystem\n` +
      `**Why:** SNAP's entire infrastructure is built on skills - any changes to skill patterns, specifications, or examples directly affect how we build and organize SNAP's capabilities.\n` +
      `**Action:** Review immediately and update SNAP's skill templates/patterns if new conventions emerge. Check if new skill categories or capabilities can be adopted.`;
  }

  // MCP - Core infrastructure
  if (titleLower.includes('mcp') || source.toLowerCase().includes('mcp')) {
    return `**SNAP Impact:** HIGH - MCP infrastructure enhancement\n` +
      `**Why:** SNAP uses MCP servers for chrome-devtools, brightdata, Ref docs, content access, and Stripe. Changes to MCP spec/docs affect our integrations.\n` +
      `**Action:** Assess compatibility with existing MCP servers in .mcp.json. Look for new MCP capabilities to expand SNAP's tooling.`;
  }

  // Commands/Slash Commands
  if (titleLower.includes('command') || titleLower.includes('slash command')) {
    return `**SNAP Impact:** HIGH - Command system update\n` +
      `**Why:** SNAP uses slash commands extensively (~/.claude/Commands/). Changes affect our command architecture and user workflows.\n` +
      `**Action:** Review for new command patterns or capabilities. Update SNAP's command templates if conventions change.`;
  }

  // Agents/Hooks
  if (titleLower.includes('agent') || titleLower.includes('hook')) {
    return `**SNAP Impact:** HIGH - Agent/Hook system change\n` +
      `**Why:** SNAP uses agents (researcher, engineer, architect, etc.) and hooks (load-context, stop-hook) as core infrastructure components.\n` +
      `**Action:** Check if this affects SNAP's agent definitions or hook configurations. Test existing agent workflows.`;
  }

  // Claude Code releases
  if (type === 'release' && source.includes('claude-code')) {
    return `**SNAP Impact:** CRITICAL - Core platform update\n` +
      `**Why:** SNAP runs on Claude Code - releases may include new features, breaking changes, or performance improvements.\n` +
      `**Action:** Review changelog carefully. Test SNAP's critical workflows. Update skills/commands if APIs changed.`;
  }

  // MCP releases
  if (type === 'release' && source.includes('MCP')) {
    return `**SNAP Impact:** HIGH - MCP protocol update\n` +
  ...
  if (titleLower.includes('plugin') || titleLower.includes('marketplace')) {
    return `**SNAP Impact:** MEDIUM - Ecosystem expansion\n` +
      `**Why:** Plugin/marketplace features could provide new capabilities to integrate into SNAP's toolkit.\n` +
      `**Action:** Explore available plugins. Assess if any solve current SNAP limitations or add valuable features.`;
  }
  ...
  if (source.includes('cookbook') || source.includes('quickstart') || source.includes('courses')) {
    return `**SNAP Impact:** MEDIUM - Implementation patterns\n` +
      `**Why:** Cookbooks/examples show best practices and patterns we can adopt in SNAP's codebase.\n` +
      `**Action:** Review for reusable patterns, especially around skills, agents, or Claude Code features. Extract learnings for SNAP.`;
  }

  // GitHub commits
  if (category === 'github' && type === 'commit') {
    return `**SNAP Impact:** LOW-MEDIUM - Code pattern review\n` +
      `**Why:** Commits may reveal implementation details, bug fixes, or patterns useful for SNAP development.\n` +
  ...
  if (titleLower.includes('doc') || type === 'docs') {
    return `**SNAP Impact:** MEDIUM - Capability discovery\n` +
      `**Why:** Doc updates often reveal new features or best practices not yet in SNAP.\n` +
      `**Action:** Review for new Claude Code features, API capabilities, or configuration options to leverage.`;
  }

  // SDK releases
  if (source.includes('sdk')) {
    return `**SNAP Impact:** LOW - SDK update\n` +
      `**Why:** SDK updates are less relevant since SNAP uses Claude Code CLI, not raw API SDKs.\n` +
      `**Action:** Note for reference. Only investigate if mentions features relevant to SNAP's agent implementations.`;
  }

  // Blog posts
  if (type === 'blog') {
    return `**SNAP Impact:** LOW-MEDIUM - Awareness\n` +
      `**Why:** Blogs announce new features and directions that may eventually affect SNAP.\n` +
      `**Action:** Skim for strategic announcements about Claude Code, Skills, or MCP. Track for future planning.`;
  }

  // Generic
  return `**SNAP Impact:** LOW - General awareness\n` +
    `**Why:** May have indirect relevance to SNAP ecosystem.\n` +
    `**Action:** Review if time permits. Low impact on SNAP's core functionality.`;
}
...
// Generate narrative analysis focused on SNAP ecosystem
function generateNarrative(updates: Update[]): string {
  ...
  let narrative = `## 📖 Executive Summary: What This Means for SNAP\n\n`;

  // Overall activity
  narrative += `Found **${updates.length} updates** across the Anthropic ecosystem in the monitored period. `;

  if (high.length > 0) {
    narrative += `**${high.length} are HIGH priority** for SNAP's infrastructure, `;
  }
  if (medium.length > 0) {
  ...
  if (skillUpdates.length > 0) {
    ...
    if (highSkills > 0) {
      ...
      narrative += `There are **${highSkills} HIGH-priority skill updates** - this is BIG because SNAP's entire architecture is built on the skills system. `;
      narrative += `Any changes to skill patterns, specifications, or conventions could require updates to SNAP's ${countSkills()} existing skills. `;
      ...
      narrative += `\n\n**→ Priority Action:** Review all skill updates immediately. Update SNAP's skill templates if conventions changed.\n\n`;
    } else {
  ...
  if (mcpUpdates.length > 0) {
    ...
    if (highMcp > 0) {
      ...
      narrative += `**${highMcp} HIGH-priority MCP updates** detected. Since SNAP uses MCP servers for chrome-devtools, brightdata, Ref, content, and Stripe, `;
      ...
      narrative += `\n\n**→ Priority Action:** Test existing MCP servers. Look for new MCP features to expand SNAP's toolkit.\n\n`;
    } else {
  ...
  if (codeUpdates.length > 0) {
    ...
    if (highCode > 0) {
      ...
      narrative += `**${highCode} HIGH-priority** Claude Code updates found. Since SNAP runs on Claude Code, platform changes can affect everything. `;
      ...
      narrative += `\n\n**→ Priority Action:** Review changelog. Test SNAP's core workflows after updating.\n\n`;
    } else {
  ...
  if (cookbookUpdates.length > 0) {
    ...
    if (skillCookbooks.length > 0) {
      narrative += `${skillCookbooks.length} specifically about skills - definitely review these for patterns to adopt in SNAP. `;
    }
    narrative += `\n\n`;
  }
  ...
  if (docUpdates.length > 5) {
    ...
    narrative += `**${docUpdates.length} documentation pages** updated. While less urgent, docs often reveal new capabilities or best practices not yet used in SNAP.\n\n`;
  }
  ...
  // Bottom line
  narrative += `### 🎯 Bottom Line for SNAP\n\n`;
  ...
  if (high.length > 5) {
    ...
    narrative += `**Quiet period** - mostly low-priority updates. Good time to focus on SNAP development rather than external changes.\n\n`;
  }
  ...
  if (skillUpdates.filter(u => u.priority === 'HIGH').length > 0) {
    topItems.push('1. **Skills system updates** - Review immediately, may affect SNAP architecture');
  }
...
function countSkills(): number {
  ...
  return 20; // Approximate based on SNAP's current skill set
}
