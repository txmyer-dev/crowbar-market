 ...
 *
 * Usage: bun ~/.claude/skills/SNAP/Tools/CreateDynamicCore.ts
 *
...
const CORE_DIR = join(HOME, ".claude/skills/SNAP");
...
function loadVariables(): Record<string, string> {
  try {
    ...
    return {
      "{DAIDENTITY.NAME}": settings.daidentity?.name || "SNAP",
      "{DAIDENTITY.FULLNAME}": settings.daidentity?.fullName || "Personal AI",
      "{DAIDENTITY.DISPLAYNAME}": settings.daidentity?.displayName || "SNAP",
    ...
    return {
      "{DAIDENTITY.NAME}": "SNAP",
      "{DAIDENTITY.FULLNAME}": "Personal AI",
      "{DAIDENTITY.DISPLAYNAME}": "SNAP",
      "{PRINCIPAL.NAME}": "User",
