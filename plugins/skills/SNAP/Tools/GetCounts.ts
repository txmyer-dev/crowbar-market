...
/**
 * GetCounts.ts - Single Source of Truth for SNAP System Counts
 *
 * PURPOSE:
 * Provides deterministic, consistent counts for SNAP system metrics.
 ...
 * - Files: All files in skills/SNAP/USER/ (recursive)
...
const SNAP_DIR = process.env.SNAP_DIR || join(HOME, ".claude");
...
function countSkills(): number {
  let count = 0;
  const skillsDir = join(SNAP_DIR, "skills");
  try {
...
function countHooks(): number {
  let count = 0;
  const hooksDir = join(SNAP_DIR, "hooks");
  try {
...
function countRatings(): number {
  const ratingsFile = join(SNAP_DIR, "MEMORY/LEARNING/SIGNALS/ratings.jsonl");
  try {
    const fs = require('fs');
...
function getCounts(): Counts {
  return {
    skills: countSkills(),
    workflows: countWorkflowFiles(join(SNAP_DIR, "skills")),
    hooks: countHooks(),
    signals: countFilesRecursive(join(SNAP_DIR, "MEMORY/LEARNING"), ".md"),
    files: countFilesRecursive(join(SNAP_DIR, "skills/SNAP/USER")),
    work: (() => {
      let count = 0;
      try {
        for (const entry of readdirSync(join(SNAP_DIR, "MEMORY/WORK"), { withFileTypes: true })) {
          if (entry.isDirectory()) count++;
        }
      } catch {}
      return count;
    })(),
    research: countFilesRecursive(join(SNAP_DIR, "MEMORY/RESEARCH"), ".md") +
              countFilesRecursive(join(SNAP_DIR, "MEMORY/RESEARCH"), ".json"),
