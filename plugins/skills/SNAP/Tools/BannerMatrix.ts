...
/**
 * BannerMatrix - Matrix Digital Rain SNAP Banner
 * Neofetch-style layout with The Matrix aesthetic
 *
 * Design:
 *   LEFT:  SNAP logo emerging from Matrix rain (Katakana cascade)
 *   RIGHT: System stats as terminal readout
 *   BOTTOM: Glitched branding + SNAP in dripping Matrix style
 *
...
// =============================================================================
// SNAP Logo in Matrix Style (ASCII that emerges from rain)
// =============================================================================

// Large SNAP letters that will "emerge" from the rain
...
// Compact SNAP logo for smaller modes
const PAI_COMPACT = [
  "┌───┐┌───┐┌─┐",
  "│ ┌─┘│ ┌─┤│ │",
  "│ │  │ ├─┤│ │",
...
function readDAIdentity(): string {
  ...
  try {
    ...
    return settings.daidentity?.displayName || settings.daidentity?.name || settings.env?.DA || "SNAP";
  } catch {
    return "SNAP";
  }
}
...
function countUserFiles(): number {
  const userDir = join(CLAUDE_DIR, "skills/SNAP/USER");
...
function createNanoBanner(stats: SystemStats): string {
  ...
  return `${d}ｱ${RESET}${g}SNAP${RESET}${d}ｲ${RESET} ${w}${stats.name}${RESET} ${g}[ON]${RESET}`;
}
 ...
 * LEFT: SNAP logo with rain | RIGHT: System stats
 * BOTTOM: SNAP dripping + branding
 */
function createNormalBanner(stats: SystemStats): string {
  ...
  // Dimensions
  const logoWidth = 40;  // Width for SNAP logo + rain
  ...
  // SNAP logo lines with rain surrounding
  ...
  const urlLine = `${d}$ git clone ${RESET}${g}https://github.com/danielmiessler/SNAP${RESET}`;
  const urlCentered = " ".repeat(Math.floor((width - 50) / 2));
  lines.push(rainOverlay(`${urlCentered}${urlLine}`.padEnd(width), 0.1));

  // Frame bottom
  const bottomFrame = `${f}${BOLD}${"=".repeat(dividerCol - 1)}[${RESET}${p}SNAP${RESET}${f}${BOLD}]${"=".repeat(width - dividerCol - 5)}${RESET}`;
