...
**The foundational pattern for SNAP extensibility and personalization**

---

## Overview

SNAP uses a consistent two-tier architecture across all configurable components:

```
SYSTEM tier  →  Base functionality, defaults, SNAP updates
USER tier    →  Personal customizations, private policies, overrides
```

This pattern enables:
- **Immediate functionality** — SNAP works out of the box with sensible defaults
- **Personal customization** — Users can override any default without modifying core files
- **Clean updates** — SNAP updates don't overwrite personal configurations
- **Privacy separation** — USER content is never synced to the public SNAP repository

---

## The Lookup Pattern

When SNAP needs configuration, it follows a cascading lookup:
...
```
skills/SNAP/SYSTEM/SNAPSECURITYSYSTEM/  # SYSTEM tier (base)
├── README.md                          # Overview
...
USER/SNAPSECURITYSYSTEM/                # USER tier (personal)
├── patterns.yaml                      # Your security rules
├── QUICKREF.md                        # Your quick reference
└── ...
```

The SecurityValidator hook checks `USER/SNAPSECURITYSYSTEM/patterns.yaml` first, falling back to `skills/SNAP/SYSTEM/SNAPSECURITYSYSTEM/patterns.example.yaml`.

### Response Format

```
skills/SNAP/SYSTEM/RESPONSEFORMAT.md  # SYSTEM tier (base format rules)
skills/SNAP/USER/RESPONSEFORMAT.md    # USER tier (personal overrides)
```
...
Private skills use the `_ALLCAPS` prefix and are never synced to public SNAP.
...
The SYSTEM tier must always provide functional defaults. A fresh SNAP installation should work immediately without requiring USER configuration.
...
The `USER/` directory is excluded from public SNAP sync. Anything in USER:
- Never appears in public SNAP repository
...
When SNAP updates, only SYSTEM tier files change. Your USER configurations remain untouched. This means:
- Safe to update SNAP without losing customizations
...
const USER_PATTERNS_PATH = paiPath('USER', 'SNAPSECURITYSYSTEM', 'patterns.yaml');
const SYSTEM_PATTERNS_PATH = paiPath('skills', 'CORE', 'SYSTEM', 'SNAPSECURITYSYSTEM', 'patterns.example.yaml');
...
Future SNAP versions may support declarative merging.
...
- `SYSTEM/SNAPSECURITYSYSTEM/` — Security system architecture and patterns
- `SYSTEM/SKILLSYSTEM.md` — Skill naming conventions (public vs private)
- `SYSTEM/SNAPSYSTEMARCHITECTURE.md` — Overall SNAP architecture
