...
The SNAP hook system is an event-driven automation infrastructure built on Claude Code's native hook support. Hooks are executable scripts (TypeScript/Python) that run automatically in response to specific events during Claude Code sessions.
...
**Use Cases:**
- Load SNAP context from `skills/SNAP/SKILL.md`
  ...
  "SessionStart": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${SNAP_DIR}/hooks/LoadContext.hook.ts"
        },
        {
          "type": "command",
          "command": "${SNAP_DIR}/hooks/CheckVersion.hook.ts"
        }
      ]
    }
  ]
...
**What They Do:**
- `LoadContext.hook.ts` - Reads `skills/SNAP/SKILL.md` and injects SNAP context as `<system-reminder>` at session start
  ...
  "SessionEnd": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${SNAP_DIR}/hooks/SessionSummary.hook.ts"
        }
      ]
    }
  ]
  ...
  "UserPromptSubmit": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${SNAP_DIR}/hooks/ExplicitRatingCapture.hook.ts"
        },
        {
          "type": "command",
          "command": "${SNAP_DIR}/hooks/ImplicitSentimentCapture.hook.ts"
        },
        {
          "type": "command",
          "command": "${SNAP_DIR}/hooks/UpdateTabTitle.hook.ts"
        }
      ]
    }
  ]
...
- **Inference:** `import { inference } from '../skills/SNAP/Tools/Inference'` → `inference({ level: 'standard', expectJson: true })`
...
- **Inference:** `import { inference } from '../skills/SNAP/Tools/Inference'` → `inference({ level: 'fast' })`
  ...
  "Stop": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${SNAP_DIR}/hooks/StopOrchestrator.hook.ts"
        }
      ]
    }
  ]
...
- Single orchestrator that delegates to specialized handlers in `${SNAP_DIR}/hooks/handlers/`:
...
- Writes learnings to `${SNAP_DIR}/MEMORY/LEARNING/<category>/YYYY-MM/` when applicable
  ...
  "SubagentStop": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${SNAP_DIR}/hooks/AgentOutputCapture.hook.ts"
        }
      ]
    }
  ]
...
- After receiving Task result, parent calls: `bun ~/.claude/skills/SNAP/Tools/UpdateTabTitle.ts "Message"`
- This avoids transcript parsing race conditions
- See `~/.claude/skills/SNAP/Tools/UpdateTabTitle.ts` for implementation
  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "${SNAP_DIR}/hooks/SecurityValidator.hook.ts"
        }
      ]
    },
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "${SNAP_DIR}/hooks/SetQuestionTab.hook.ts"
        }
      ]
    }
  ]
...
**Key Variables:**
- `SNAP_DIR` - SNAP installation directory (typically `~/.claude`)
- Hook scripts reference `${SNAP_DIR}` in command paths
...
- Central to the SNAP install wizard
    ...
    "HookEventName": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "${SNAP_DIR}/hooks/my-hook.ts --arg value"
          }
        ]
      }
    ]
...
await sendEventToObservability({
  source_app: getSourceApp(),           // 'SNAP' or agent name
...
**Full Documentation:** See `~/.claude/skills/SNAP/SYSTEM/TERMINALTABS.md`
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "${SNAP_DIR}/hooks/my-custom-hook.ts"
          }
        ]
      }
    ]
...
- Use `${SNAP_DIR}` in settings.json for portability
- Access in code via `process.env.SNAP_DIR`
...
2. Is path correct in settings.json? Use `${SNAP_DIR}/hooks/...`
...
2. Is voice_id correct? See `skills/SNAP/SKILL.md` for mappings
3. Is message format correct? `{"message":"...", "voice_id":"...", "title":"..."}`
4. Is ElevenLabs API key in `${SNAP_DIR}/.env`?
...
1. **Added CAPTURE field to response format** (see `~/.claude/skills/SNAP/SKILL.md`)
...
**Tracking:** Documented in `~/.claude/skills/SNAP/SKILL.md` (History Capture System section)
...
**Check:**
1. Does `~/.claude/skills/SNAP/SKILL.md` exist?
2. Is `LoadContext.hook.ts` executable?
3. Is `SNAP_DIR` env variable set correctly?
...
- File not found → Check `SNAP_DIR` environment variable
...
```json
{
  "Stop": [
    {
      "hooks": [
        { "command": "${SNAP_DIR}/hooks/StopOrchestrator.hook.ts" }  // Single orchestrator
      ]
    }
  ]
}
...
- **History/Memory:** `~/.claude/skills/SNAP/SYSTEM/MEMORYSYSTEM.md`
...
Path: ~/.claude/skills/SNAP/Tools/Inference.ts
Import: import { inference } from '../skills/SNAP/Tools/Inference'
...
LoadContext.hook.ts             SNAP context loading
...
### `skills/SNAP/Tools/Inference.ts`
Unified AI inference with three run levels.

```typescript
import { inference } from '../skills/SNAP/Tools/Inference';
