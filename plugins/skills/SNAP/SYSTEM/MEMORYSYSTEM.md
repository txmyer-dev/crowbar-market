...
This is the actual "firehose" - every message, tool call, and response. SNAP leverages this native storage rather than duplicating it.
...
The `ISC.json` file captures the Ideal State Criteria from SNAP Algorithm execution. This enables:
...
**Structure:**
- `LEARNING/SYSTEM/YYYY-MM/` - SNAP/tooling learnings (infrastructure issues)
...
bun run ~/.claude/skills/SNAP/Tools/FailureCapture.ts --migrate
```
...
bun run ~/.claude/skills/SNAP/Tools/SessionHarvester.ts --recent 10

# Generate pattern synthesis
bun run ~/.claude/skills/SNAP/Tools/LearningPatternSynthesis.ts --week
```
...
- **Architecture:** `SNAPSYSTEMARCHITECTURE.md`
