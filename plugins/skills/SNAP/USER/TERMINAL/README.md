...
- `ZSHRC` - Shell configuration with SNAP alias
...
3. **Copy configuration:**
   ...
   cp ~/.claude/skills/SNAP/USER/TERMINAL/kitty.conf ~/.config/kitty/
   ```

4. **Add shell aliases:**
   Add the contents of `ZSHRC` to your `~/.zshrc`:
   ```bash
   cat ~/.claude/skills/SNAP/USER/TERMINAL/ZSHRC >> ~/.zshrc
   source ~/.zshrc
   ```
...
The SNAP hook system dynamically updates tab titles and colors to provide real-time visual feedback about what the AI is doing. This creates a glanceable status system across multiple terminal tabs.
...
With multiple terminal tabs running different SNAP sessions, you can instantly see:
