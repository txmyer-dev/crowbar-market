 ...
 *
 * Manages the ElevenLabs TTS voice server for SNAP notifications.
 *
 * Usage:
 ...
 *   bun VoiceServerManager.ts test "Hello from SNAP"
 *
...
const VOICE_SERVER_PATH = join(process.env.HOME || "", ".claude/VoiceServerV1");
const PORT = 8888;
...
async function test(message?: string): Promise<void> {
  const testMessage = message || "Voice server test from SNAP";
