...
const SNAP_DIR = join(process.env.HOME || process.env.USERPROFILE || homedir(), '.claude');
const CREATE_UPDATE_SCRIPT = join(SNAP_DIR, 'skills/_SYSTEM/Tools/CreateUpdate.ts');
...
function generateDescriptiveTitle(changes: FileChange[]): string {
  ...
  const hasPAISystem = paths.some(p => p.includes('SNAP/SYSTEM/'));
  const hasPAIUser = paths.some(p => p.includes('SNAP/USER/'));
...
async function generateNarrativeWithAI(
  ...
  const prompt = `You are analyzing a Claude Code session to generate documentation for a SNAP (Personal AI Infrastructure) system update.
...
async function generateVerboseNarrative(
  ...
  const skillContext = skillNames.size > 0 ? [...skillNames].slice(0, 3).join(', ') : 'SNAP system';
