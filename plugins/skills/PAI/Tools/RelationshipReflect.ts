...
const SNAP_DIR = process.env.SNAP_DIR || join(process.env.HOME || process.env.USERPROFILE || homedir(), '.claude');
...
function loadRecentNotes(daysBack: number = 7): RelationshipNote[] {
  ...
  for (const monthStr of months) {
    const monthDir = join(SNAP_DIR, 'MEMORY/RELATIONSHIP', monthStr);
...
function loadRecentRatings(daysBack: number = 7): Array<{ rating: number; date: string }> {
  const ratingsPath = join(SNAP_DIR, 'MEMORY/LEARNING/SIGNALS/ratings.jsonl');
...
function parseOpinions(): Map<string, { confidence: number; section: string }> {
  ...
  const opinionsPath = join(SNAP_DIR, 'skills/SNAP/USER/OPINIONS.md');
...
function checkMilestones(notes: RelationshipNote[]): string[] {
  ...
  const storyPath = join(SNAP_DIR, 'skills/SNAP/USER/OUR_STORY.md');
