...
/**
 * SNAP Banner - Dynamic Multi-Design Neofetch Banner
...
function getStats(): SystemStats {
  ...
  let repoUrl = "github.com/danielmiessler/SNAP";
  try {
    const settings = JSON.parse(readFileSync(join(CLAUDE_DIR, "settings.json"), "utf-8"));
    name = settings.daidentity?.displayName || settings.daidentity?.name || "SNAP";
  ...
  try {
    const latestPath = join(CLAUDE_DIR, "skills/SNAP/Components/Algorithm/LATEST");
...
function createNavyBanner(stats: SystemStats, width: number): string {
  ...
  // SNAP logo - 2x scale (20 wide × 10 tall), same proportions
  ...
  const infoLines = [
    ...
    `${C.navy}\u2B22${RESET}  ${C.slate}SNAP${RESET}       ${C.silver}v${stats.paiVersion}${RESET}`,                            // ⬢ hexagon (tech/AI)
  ...
  // Header: SNAP (in logo colors) | Personal AI Infrastructure
  ...
  const headerLen = 33; // "SNAP | Personal AI Infrastructure"
...
function createElectricBanner(stats: SystemStats, width: number): string {
  ...
  // SNAP logo - matching reference image exactly
  const B = "\u2588";
  const logo = [
  ...
  const infoLines = [
    ...
    `${P.neonBlue}${SYM.target}${RESET} ${P.glow}OS${RESET}         ${P.electric}SNAP v${stats.paiVersion}${RESET}`,
...
function createTealBanner(stats: SystemStats, width: number): string {
  ...
  const infoLines = [
    ...
    `${P.mediumTeal}${SYM.wave}${RESET} ${P.paleAqua}OS${RESET}         ${P.aquamarine}SNAP v${stats.paiVersion}${RESET}`,
...
function createIceBanner(stats: SystemStats, width: number): string {
  ...
  const infoLines = [
    ...
    `${P.iceBlue}${SYM.crystal}${RESET} ${P.frost}OS${RESET}         ${P.white}SNAP v${stats.paiVersion}${RESET}`,
...
function createNavyMediumBanner(stats: SystemStats, width: number): string {
  ...
  const infoLines = [
    `${C.slate}"${RESET}${C.lightBlue}${stats.catchphrase}${RESET}${C.slate}..."${RESET}`,
    `${C.steel}${BOX.h.repeat(24)}${RESET}`,
    `${C.navy}\u2B22${RESET}  ${C.slate}SNAP${RESET}       ${C.silver}v${stats.paiVersion}${RESET}`,
...
function createNavyCompactBanner(stats: SystemStats, width: number): string {
  ...
  const infoLines = [
    ...
    `${C.navy}\u2B22${RESET} ${C.slate}SNAP${RESET} ${C.silver}v${stats.paiVersion}${RESET} ${C.navy}\u2699${RESET} ${C.silver}v${stats.algorithmVersion}${RESET}`,
