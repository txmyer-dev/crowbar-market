---
name: SNAP
description: Personal AI Infrastructure core. The authoritative reference for how SNAP works.
---
<!--
  🔨 GENERATED FILE - Do not edit directly
  Edit:   ~/.claude/skills/SNAP/Components/
  Build:  bun ~/.claude/skills/SNAP/Tools/CreateDynamicCore.ts
  Built:  24 February 2026 00:05:42
-->

# Intro to SNAP

The SNAP system is designed to magnify human capabilities. It is a general problem-solving system that uses the SNAP Algorithm.
...
`curl -s -X POST http://localhost:8888/notify -H "Content-Type: application/json" -d '{"message": "Entering the SNAP Algorithm Observe phase"}'`
...
**Step 3.5 — ISC Splitting Test (v4-upgrade):** Before writing ANY criterion, apply these 4 mandatory decomposition tests:
  1. **"And"/"With" test** — If the criterion contains "and", "with", or "including" joining two verifiable things, SPLIT into separate criteria.
  2. **Independent failure test** — Can part A pass while part B fails? If YES → they are separate criteria.
  3. **Scope word test** — If criterion contains "all", "every", "complete", enumerate what that actually means. Each enumerated item may be its own criterion.
  4. **Domain boundary test** — Does the criterion cross UI/API/data/logic boundaries? If YES → one criterion per boundary.
  Any criterion that fails a test MUST be split before proceeding. This is mechanical, not judgment-based.
...
🗣️ Ekko: [Spoken summary between 12-24 words.]
```
...
### ISC Count Gate (v4-upgrade — hard minimums per effort tier)

You CANNOT exit OBSERVE with fewer criteria than the effort tier floor. This is a HARD GATE — not a suggestion.

| Effort Tier | Minimum ISC Count | Rationale |
|-------------|-------------------|-----------|
| Instant | 0 (no ISC) | No phases at all |
| Fast | 4 | Minimum viable verification |
| Standard | 8 | Sufficient coverage for bounded tasks |
| Extended | 16 | Multi-file changes need thorough coverage |
| Advanced | 24 | Substantial work with cross-cutting concerns |
| Deep | 40 | Complex design needs granular verification |
| Comprehensive | 64 | Full product scope demands exhaustive criteria |

**Enforcement:** After QG1 Count check, verify actual count >= tier floor. If under, you are BLOCKED. Generate more criteria — decompose existing ones using the ISC Splitting Test, discover missing domains, or examine edge cases. Do not proceed with under-populated ISC.
...
Critical SNAP documentation organized by domain. Load on-demand based on context.

| Domain | Path | Purpose |
|--------|------|---------|
| **System Architecture** | `SYSTEM/SNAPSYSTEMARCHITECTURE.md` | Core SNAP design and principles |
...
| **Agent System** | `SYSTEM/SNAPAGENTSYSTEM.md` | Agent types, spawning, delegation |
