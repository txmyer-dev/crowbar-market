...
**Authoritative reference for agent routing in SNAP. Three distinct systems exist—never confuse them.**

---

## 🚨 THREE AGENT SYSTEMS — CRITICAL DISTINCTION

SNAP has three agent systems that serve different purposes. Confusing them causes routing failures.
...
```
Multiple problems? ──yes──> Are they independent? ──yes──> Can agents work without shared state? ──yes──> PARALLEL DISPATCH
       │                         │                              │
...
**Example — Real dispatch for 3 failing test files:**

```
Agent 1:
  SCOPE: Fix agent-tool-abort.test.ts (3 failures)
  GOAL: All 3 tests pass — timing/race condition issues
...
1. **Read each summary** — Understand what each agent found and changed
2. **Check for conflicts** — Did any agents edit the same files?
3. **Run full test suite** — All fixes must work together, not just individually
4. **Spot check** — Agents can make systematic errors; verify the logic
