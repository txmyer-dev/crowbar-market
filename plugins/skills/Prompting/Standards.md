...
## Native Fabric Patterns in SNAP

**Location:** `~/.claude/skills/SNAP/Tools/fabric/Patterns/`

SNAP maintains a local copy of all Fabric patterns for native execution. Instead of spawning the `fabric` CLI for every pattern-based task, the system reads and applies patterns directly as prompts.
...
```bash
~/.claude/skills/SNAP/Tools/fabric/update-patterns.sh
```

This pulls upstream updates via `fabric -U` and syncs to SNAP's local copy.
...
**Foundation:** Based on Anthropic's official `{{variable}}` syntax, industry patterns (LangChain, Handlebars, DSPy), and SNAP's unique skill architecture.

**Philosophy:** Templates enable prompts to write prompts—dynamic composition where structure is fixed but content is parameterized. This is core SNAP DNA.

---

## Core Syntax

SNAP uses Handlebars notation for template variables:
