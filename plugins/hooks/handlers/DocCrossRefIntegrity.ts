...
import type { ParsedTranscript } from '../../skills/SNAP/Tools/TranscriptParser';
...
const SYSTEM_DIR = paiPath('skills', 'SNAP');
...
function isSystemDocModified(modifiedFiles: Set<string>): boolean {
  for (const path of modifiedFiles) {
    if (path.includes('skills/SNAP/') && path.endsWith('.md')) return true;
  }
  return false;
}
...
/**
 * Check if ANY meaningful SNAP system file was modified.
...
function isSystemFileModified(modifiedFiles: Set<string>): boolean {
  ...
  for (const filePath of modifiedFiles) {
    ...
    const relPath = filePath.startsWith(SNAP_DIR)
      ? filePath.slice(SNAP_DIR.length + 1)
      : filePath;

    // Must be within SNAP directory
    if (filePath.startsWith('/') && !filePath.startsWith(SNAP_DIR)) continue;

    // Skip excluded paths
    if (EXCLUDED.some(ex => relPath.includes(ex))) continue;

    // Match meaningful system files (skills includes SNAP root MDs, SYSTEM docs, USER dir, Components, Tools, Workflows)
    ...
    if (relPath.includes('SNAP/Components/') && relPath.endsWith('.md')) return true;
...
function checkSystemDocRefs(docsToCheck: string[], systemDocsOnDisk: Set<string>): DriftItem[] {
  ...
  const sysDocRefRegex = /(?:`|'|")(?:~\/\.claude\/skills\/SNAP\/)?SYSTEM\/([\w/]+\.md)(?:`|'|")/g;
  ...
  for (const docFile of docsToCheck) {
    ...
    while ((match = sysDocRefRegex.exec(content)) !== null) {
      const refTarget = match[1]; // e.g., "SNAPSYSTEMARCHITECTURE.md" or "SNAPSECURITYSYSTEM/ARCHITECTURE.md"
      const targetBasename = basename(refTarget);
      // For nested paths like SNAPSECURITYSYSTEM/ARCHITECTURE.md, check file exists
...
export async function handleDocCrossRefIntegrity(
  ...
  // Run if ANY meaningful SNAP system file was modified (skills, hooks, tools, config, components, workflows, SYSTEM docs)
  ...
  for (const path of modifiedFiles) {
    if (path.includes('skills/SNAP/') && path.endsWith('.md')) {
