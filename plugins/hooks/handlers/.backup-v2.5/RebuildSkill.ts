...
export async function handleRebuildSkill(): Promise<void> {
  const CORE_DIR = join(process.env.HOME || process.env.USERPROFILE || homedir(), '.claude', 'skills', 'SNAP');
...
function rebuild(buildScript: string): void {
  const result = spawnSync('bun', [buildScript], {
    cwd: join(process.env.HOME || process.env.USERPROFILE || homedir(), '.claude', 'skills', 'SNAP'),
    encoding: 'utf-8',
  });
