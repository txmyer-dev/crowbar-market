 ...
 *
 * Detects SNAP system changes from the transcript and spawns background
...
const INTEGRITY_SCRIPT = paiPath('skills', 'SNAP', 'Tools', 'IntegrityMaintenance.ts');
...
function updateIntegrityState(changes: FileChange[]): void {
  try {
    ...
    // Updated integrity state file
  } catch (error) {
    // Failed to update integrity state - non-critical
  }
}
...
function spawnIntegrityMaintenance(
  ...
  try {
    ...
    if (!existsSync(INTEGRITY_SCRIPT)) {
      // IntegrityMaintenance.ts not found
      return;
    }
    ...
    // Pre-computed: title, significance, change type
    ...
    // Spawned IntegrityMaintenance background process
  } catch (error) {
    // Failed to spawn IntegrityMaintenance - non-critical
  }
}
 ...
 * 2. Filters for SNAP system paths (excludes WORK/, LEARNING/, scratch/)
...
export async function handleSystemIntegrity(
  ...
  // Background integrity check started
}
