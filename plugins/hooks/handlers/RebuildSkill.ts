#!/usr/bin/env bun
/**
 * RebuildSkill.ts - Auto-rebuild SNAP SKILL.md from Components
 *
 * PURPOSE:
...
export async function handleRebuildSkill(): Promise<void> {
  const CORE_DIR = join(process.env.HOME!, '.claude/skills/SNAP');
...
function rebuild(buildScript: string): Promise<void> {
  return new Promise((resolve) => {
    ...
    const child = spawn('bun', [buildScript], {
      cwd: join(process.env.HOME!, '.claude/skills/SNAP'),
