...
import { captureFailure } from '../skills/SNAP/Tools/FailureCapture';
...
const ALGO_VERSION = (() => {
  try {
    const paiDir = process.env.SNAP_DIR || join(process.env.HOME!, '.claude');
    return readFileSync(join(paiDir, 'skills', 'SNAP', 'Components', 'Algorithm', 'LATEST'), 'utf-8').trim();
  } catch { return 'v?.?.?'; }
})();
...
const BASE_DIR = process.env.SNAP_DIR || join(process.env.HOME!, '.claude');
