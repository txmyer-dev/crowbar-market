...
// 1. USER/SNAPSECURITYSYSTEM/patterns.yaml (user's custom rules)
// 2. skills/SNAP/SYSTEM/SNAPSECURITYSYSTEM/patterns.example.yaml (default template)
const USER_PATTERNS_PATH = paiPath('USER', 'SNAPSECURITYSYSTEM', 'patterns.yaml');
const SYSTEM_PATTERNS_PATH = paiPath('skills', 'SNAP', 'SYSTEM', 'SNAPSECURITYSYSTEM', 'patterns.example.yaml');
...
function handleBash(input: HookInput): void {
  ...
  switch (result.action) {
    case 'block':
      ...
      console.error(`[SNAP SECURITY] 🚨 BLOCKED: ${result.reason}`);
    ...
    case 'confirm':
      ...
      console.log(JSON.stringify({
        decision: 'ask',
        message: `[SNAP SECURITY] ⚠️ ${result.reason}\n\nCommand: ${command.slice(0, 200)}\n\nProceed?`
      }));
      break;
    ...
    case 'alert':
      ...
      console.error(`[SNAP SECURITY] ⚠️ ALERT: ${result.reason}`);
      console.error(`Command: ${command.slice(0, 100)}`);
      console.log(JSON.stringify({ continue: true }));
      break;
