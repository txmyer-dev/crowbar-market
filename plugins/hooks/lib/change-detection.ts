/**
 * change-detection.ts - Utilities for detecting SNAP system changes
 *
...
const STATE_FILE = join(SNAP_DIR, 'MEMORY', 'STATE', 'integrity-state.json');
...
/**
 * Normalize an absolute path to relative (to SNAP_DIR).
 */
function normalizeToRelativePath(absolutePath: string): string {
  if (absolutePath.startsWith(SNAP_DIR)) {
    return relative(SNAP_DIR, absolutePath);
  }
  return absolutePath;
}
...
/**
 * Categorize a file path by its location in the SNAP system.
 */
export function categorizeChange(path: string): ChangeCategory | null {
  ...
  // Check if path is within SNAP directory
  const absolutePath = path.startsWith('/') ? path : join(SNAP_DIR, path);
  if (!absolutePath.startsWith(SNAP_DIR)) {
    return null;
  }
  ...
  if (path.includes('skills/')) {
    ...
    if (path.includes('skills/SNAP/')) return 'core-system';
    return 'skill';
  }
...
export function isSignificantChange(changes: FileChange[]): boolean {
  // Filter to only SNAP system changes
...
export function generateDescriptiveTitle(changes: FileChange[]): string {
  ...
  for (const p of paths) {
    ...
    if (match && match[1] !== 'SNAP') skillNames.add(match[1]);
  }
  ...
  const hasCoreSystem = paths.some(p => p.includes('skills/SNAP/'));
  const hasCoreUser = paths.some(p => p.includes('SNAP/USER/'));
  ...
  // Core system changes
    ...
    const docNames = paths
      .filter(p => p.includes('skills/SNAP/'))
      .map(p => basename(p, '.md'));
    if (docNames.length === 1) {
      title = `${docNames[0]} Documentation Updated`;
    } else {
      title = 'SNAP System Documentation Updated';
    }
  }
  // Core user changes
  else if (hasCoreUser) {
    const docNames = paths
      .filter(p => p.includes('SNAP/USER/'))
      .map(p => basename(p, '.md'));
    if (docNames.length === 1) {
